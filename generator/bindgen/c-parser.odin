...
parse :: proc(bytes : []u8, options_: ParserOptions, loc := #caller_location) -> Nodes {
    options := options_;
...
parse_standard_type :: proc(data : ^ParserData) -> StandardType {
    ...
    else if token == "bool" { eat_token(data); return StandardType.Bool; }
